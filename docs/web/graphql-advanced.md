...
```
**Tool:** curl · **Platform:** Any · **Tags:** GraphQL, Baseline · **Context:** No auth

## Query type fields
```bash
curl -sk https://<TARGET>/graphql -H 'Content-Type: application/json' -d '{"query":"{__schema{queryType{fields{name}}}}"}'
```
**Tool:** curl · **Platform:** Any · **Tags:** GraphQL, Introspection · **Context:** No auth
...
```
**Tool:** curl · **Platform:** Any · **Tags:** GraphQL, Mutation · **Context:** No auth

## Types and fields
```bash
curl -sk https://<TARGET>/graphql -H 'Content-Type: application/json' -d '{"query":"{__schema{types{name fields{name}}}}"}'
```
**Tool:** curl · **Platform:** Any · **Tags:** GraphQL, Schema · **Context:** No auth

## GET-based GraphQL
```bash
curl -skG https://<TARGET>/graphql --data-urlencode 'query={__typename}'
```
**Tool:** curl · **Platform:** Any · **Tags:** GraphQL, GET · **Context:** No auth
