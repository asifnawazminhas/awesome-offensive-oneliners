...
```
**Tool:** curl · **Platform:** Any · **Tags:** SOAP, WSDL · **Context:** No auth

## WSDL path probe
```bash
curl -skI https://<TARGET>/service.wsdl
```
**Tool:** curl · **Platform:** Any · **Tags:** WSDL, Discovery · **Context:** No auth

## Discover WSDL references
```bash
curl -sk https://<TARGET>/ | grep -Eoi 'https?://[^" ]+\.wsdl|[^" ]+\?wsdl' | sort -u
```
**Tool:** curl/grep · **Platform:** Linux/macOS · **Tags:** WSDL, Discovery · **Context:** No auth

## SOAPAction baseline
```bash
curl -sk -X POST https://<TARGET>/service -H 'Content-Type: text/xml; charset=utf-8' -H 'SOAPAction: "<ACTION>"' --data-binary @request.xml
```
**Tool:** curl · **Platform:** Any · **Tags:** SOAP, SOAPAction · **Context:** No auth
...
```
**Tool:** curl · **Platform:** Any · **Tags:** SOAP, SOAP 1.2 · **Context:** No auth
